...
void vector_init(Vector *vec, usize element_size, usize initial_capacity)
{
    vec->element_size = element_size;
    vec->capacity = initial_capacity > 8 ? initial_capacity : 8;
    vec->size = 0;
    vec->data = alloc(vec->capacity * element_size);
}
